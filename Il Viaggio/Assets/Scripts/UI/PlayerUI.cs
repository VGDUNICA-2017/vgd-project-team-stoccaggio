...
public class PlayerUI : MonoBehaviour {
    ...
    private OnTransitionEnd cinematicEndHandler;
    ...
    {
        if (Input.GetKeyDown(KeyCode.Q) && SceneController.CurrentScene != null)
                SceneController.CurrentScene.EquipPrevItem();

        if (Input.GetKeyDown(KeyCode.E) && SceneController.CurrentScene != null)
    ...
    #region Cinematic Panel

    public void OpenCinematicPanel(string richText, OnTransitionEnd exitButtonHandler)
    {
        cinematicPanel.SetActive(true);

        cinematicPanel.GetComponentsInChildren<Text>()[0].text = richText;

        cinematicEndHandler = exitButtonHandler;

        cinematicPanel.GetComponent<Animation>().Play("openTransition");
    }

    private void cinematicPanelController()
    {
        if (cinematicPanel.activeSelf && Input.GetKeyDown(KeyCode.E))
        {
    ...
    public void OpenTitlePanel(OnTransitionEnd onTransitionEnd)
    {
        titlePanel.SetActive(true);
        titlePanel.GetComponent<Animation>().Play("openTransition");

        StartCoroutine(titleTransitionHandler(onTransitionEnd));
    }

    private IEnumerator titleTransitionHandler(OnTransitionEnd onTransitionEnd)
    {
        yield return new WaitForSeconds(10);

        titlePanel.GetComponent<Animation>().Play("closeTransition");
