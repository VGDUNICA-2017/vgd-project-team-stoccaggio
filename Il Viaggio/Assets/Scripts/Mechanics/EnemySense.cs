        ...
        if (!playerCaught)
        {
            ...
            if (fieldOfFeel() || fieldOfView())
            {
                ...
                else if (!alert)
                {
                    ...
                    SceneController.CurrentScene.NpcSpeak(npcName, "Mi è sembrato di sentire un rumore...");
                }
                else
                {
                    ...
                    if (!playerCaught)
                    {
                        ...
                        if (Time.time >= alertTimeout)
                        {
                            ...
                            if (SceneManager.GetActiveScene().name != "Astronave")
                                SceneController.CurrentScene.NpcSpeak(npcName, "Allarme! C'è un intruso! Raggiungetemi alle coordinate che vi sto inviando!");
                            else
                                SceneController.CurrentScene.NpcSpeak(npcName, "Non ci posso credere! Ragazzi aiutatemi!");
